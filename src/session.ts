...
function getThinkingLevel(message: string): number {
  ...
  // Default: use configured default thinking level (10000 = normal thinking)
...
class ClaudeSession {
  ...
  sessionPath: string = SESSION_FILE; // Default to legacy path
  ...
  private lastWarningLevel = 0; // Track which warning level was last shown
  ...
  /**
   * Set the session file path for this conversation context.
   * Call this before sendMessageStreaming() to use Topic-specific sessions.
   */
      ...
      for await (const event of queryInstance) {
        ...
        if (event.type === "result") {
          ...
          if ("usage" in event && event.usage) {
            ...
            const percentUsed = (totalTokens / 200000) * 100;

            console.log(
              `Usage: in=${u.input_tokens} out=${u.output_tokens} cache_read=${
                u.cache_read_input_tokens || 0
              } cache_create=${u.cache_creation_input_tokens || 0} | Total: ${totalTokens} (${percentUsed.toFixed(1)}%, ${this.messageCount} msgs)`
            );

            // Multi-level warnings with user notification
            ...
            if (warningLevel > this.lastWarningLevel) {
              ...
              this.sendContextWarning(warningLevel, totalTokens, percentUsed, statusCallback);
            }

            // Auto-save context summary at 50% usage for better resume
            if (percentUsed >= 50 && !this.contextSummary) {
              this.contextSummary = `[Context preserved at 50% usage - ${new Date().toISOString()}]\nSession: ${this.sessionId?.slice(0, 8)}...\nMessages: ${this.messageCount}\nImportant: Check MEMORY.md for full context if needed.`;
              console.log("📝 Context summary auto-saved for future resume");
            }
          }
        }
      }
   ...
   */
  private getWarningLevel(percentUsed: number): number {
    if (percentUsed >= 95) return 3; // Critical
    if (percentUsed >= 85) return 2; // High
    if (percentUsed >= 75) return 1; // Moderate
    return 0; // Normal
  }

  /**
   * Send context warning to user via status callback.
   */
  private async sendContextWarning(
    level: number,
    totalTokens: number,
    percentUsed: number,
    statusCallback: StatusCallback
    ...
    switch (level) {
      case 1: // 75%
        icon = "⚡";
        message = `${icon} Context usage: ${percentUsed.toFixed(1)}% (${totalTokens.toLocaleString()}/200k tokens)\n\n💡 Consider using /new soon for better responses.`;
        console.warn(`⚡ Context at 75%: ${totalTokens}/200000 tokens`);
        break;

      case 2: // 85%
        icon = "⚠️";
        message = `${icon} Context usage high: ${percentUsed.toFixed(1)}% (${totalTokens.toLocaleString()}/200k tokens)\n\n🔄 Recommend using /new to start fresh and avoid answer quality degradation.`;
        console.warn(`⚠️  Context at 85%: ${totalTokens}/200000 tokens - recommend /new`);
        break;

      case 3: // 95%
        icon = "🚨";
        message = `${icon} CRITICAL: Context almost full at ${percentUsed.toFixed(1)}% (${totalTokens.toLocaleString()}/200k tokens)\n\n❗ Strongly recommend /new NOW. Responses may become unreliable.`;
        logLevel = "error";
        console.error(`🚨 CRITICAL: Context at 95%: ${totalTokens}/200000 tokens - /new REQUIRED`);
        break;
    }

    if (message) {
      // Send warning to user as a "text" event (will appear as separate message)
      try {
        await statusCallback("text", message);
      } catch (error) {
  ...
  saveSession(): void {
    ...
    try {
      ...
      // Save
      Bun.write(this.sessionPath, JSON.stringify(history, null, 2));
  ...
  private loadSessionHistory(): SessionHistory {
    try {
      const file = Bun.file(this.sessionPath);
      if (!file.size) {
        return { sessions: [] };
      }

      const text = readFileSync(this.sessionPath, "utf-8");
