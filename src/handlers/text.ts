...
export async function handleText(ctx: Context): Promise<void> {
  ...
  logMessage(chatId, username, "text", message);

  // 0. Extract Topic info and send eye reaction
  const topicInfo = getTopicInfo(ctx);
  const topicId = getTopicIdentifier(topicInfo);

  console.log(`📩 Message from ${topicId}: "${message.slice(0, 50)}${message.length > 50 ? '...' : ''}"`);

  // Set Topic-specific session path
  const sessionPath = getSessionPath(topicInfo.chatId, topicInfo.threadId);
  session.setSessionPath(sessionPath);
  console.log(`📂 Session path: ${sessionPath}`);

  // Send eye reaction (non-blocking, runs in background)
  // ⚠️ TEMPORARILY DISABLED: Telegram API returns REACTION_INVALID for 👁️ emoji
  // sendEyeReaction(ctx).catch((e) =>
  //   console.warn("Eye reaction failed:", e)
  // );
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      logError(error, "processing text message");
